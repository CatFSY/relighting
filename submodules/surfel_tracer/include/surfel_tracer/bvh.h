...
public:
    ...
    virtual void gaussian_trace_forward(
        ...
        const float alpha_min, const float transmittance_min, const int deg, const int max_coeffs, const bool back_culling, const int trace_mode, int* diagnostics, const bool diagnostics_enabled, cudaStream_t stream) = 0;
    ...
    virtual void build_bvh(const float* triangles, int n_triangles, bool static_bvh, cudaStream_t stream) = 0;
    virtual void update_bvh(const float* triangles, int n_triangles, cudaStream_t stream) = 0;
    virtual void build_ias(
        const float* triangles, const int64_t* triangle_offsets,
        const bool* static_flags, const float* transforms,
        int n_instances, cudaStream_t stream) = 0;
    virtual void update_ias_gas(
        int instance_index, const float* triangles, int n_triangles,
        cudaStream_t stream) = 0;
    virtual void update_ias_transforms(
        const float* transforms, int n_instances, cudaStream_t stream) = 0;
