...
#define OPTIX_CHECK_THROW(x)                                                                                 \
	do {                                                                                                     \
		...
		if (res != OPTIX_SUCCESS) {                                                                          \
			throw std::runtime_error(std::string("Optix call '" #x "' failed with code ") +                \
...
namespace optix {
	...
	public:
		Program(const char* data, size_t size, OptixDeviceContext optix,
			const char* raygen_entry = "__raygen__rg",
			unsigned int traversable_graph_flags =
				OPTIX_TRAVERSABLE_GRAPH_FLAG_ALLOW_SINGLE_GAS,
			unsigned int max_traversable_depth = 1) {
			...
			OptixProgramGroup hitgroup_prog_group = nullptr;
			{
				...
				raygen_prog_group_desc.raygen.entryFunctionName = raygen_entry;
