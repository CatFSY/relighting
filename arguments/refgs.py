...
class ModelParams(ParamGroup): 
    def __init__(self, parser, sentinel=False):
        ...
        # 与 arguments/__init__.py 的 ModelParams.mask_dir 保持一致
        self.mask_dir = ""
        # 可选的 DiffusionRenderer 材质目录。Stage1 伪 GT normal 监督还需
        # 显式传 --normal_supervision 和非零 --lambda_normal_gt。
...
class PipelineParams(ParamGroup):
    def __init__(self, parser):
        ...
        # visibility 可视化 trace 参数（与 Stage2 arguments/__init__.py 保持一致，
        # 供 save_training_vis 复刻 render_ir 的 per-pixel visibility 计算）
...
class OptimizationParams(ParamGroup):
    def __init__(self, parser):
        ...
        # Stage1 伪 GT normal 监督默认关闭。必须显式传
        # --normal_supervision，并用 --lambda_normal_gt 设置监督权重。
        self.normal_supervision = False
