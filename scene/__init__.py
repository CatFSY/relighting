    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0]):
        ...
        if os.path.exists(os.path.join(args.source_path, "sparse")):
            scene_info = sceneLoadTypeCallbacks["Colmap"](args.source_path, args.images, args.eval,
                                                           mask_dir=getattr(args, "mask_dir", "") or None,
                                                           pred_material_dir=getattr(args, "pred_material_dir", "") or None,
                                                           white_background=args.white_background)
        elif os.path.exists(os.path.join(args.source_path, "camera.json")) and \
                not os.path.exists(os.path.join(args.source_path, "transforms_train.json")):
            print("Found camera.json (no transforms_train.json), assuming arb_render_data set!")
            scene_info = sceneLoadTypeCallbacks["Arb"](args.source_path, args.white_background,
                                                       args.eval, getattr(args, "arb_env_idx", 0),
                                                       pred_material_dir=getattr(args, "pred_material_dir", "") or None)
            ...
            elif "Synthetic4Relight" in args.source_path or os.path.exists(os.path.join(args.source_path, "train", "000_rgb.exr")):
