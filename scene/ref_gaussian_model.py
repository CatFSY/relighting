...
class RefGaussianModel:
    ...
    def __init__(self, sh_degree : int):
        ...
        # GS 级 BVH tracer：供 visualize 遮挡性(AO)图使用；训练本身不依赖。
        # 与 GaussianModel(Stage2) 同款，便于对比两个阶段的遮挡。
    ...
    # ---- GS 级 BVH tracer（从 GaussianModel 移植，仅用于 visualize 遮挡图） ----
    ...
    def trace(self, rays_o, rays_d, features=None, camera_center=None, back_culling=False):
        ...
        color, normal, feature, depth, alpha = self.gaussian_tracer.trace(
            rays_o, rays_d, means3D, opacity, ru, rv, normals, features, shs,
