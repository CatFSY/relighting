...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, image, gt_alpha_mask,
                 ...
                 gt_albedo=None, gt_orm=None, gt_normal=None,
                 ):
        ...
        # GT material maps (arb_render_data), both in RAW LINEAR space:
        # gt_albedo (3,H,W) linear albedo (sRGB PNG converted at load time),
        # gt_orm (3,H,W) linear [occlusion, roughness, metallic]
        # gt_normal (3,H,W) world-space normal, encoded as (n*0.5+0.5)
        self.gt_albedo = gt_albedo.to(self.data_device) if gt_albedo is not None else None
        self.gt_orm = gt_orm.to(self.data_device) if gt_orm is not None else None
        self.gt_normal = gt_normal.to(self.data_device) if gt_normal is not None else None
