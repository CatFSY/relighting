...
from utils.general_utils import safe_state, flip_align_view, safe_normalize
import numpy as np
from tqdm import tqdm
from utils.graphics_utils import cosine_hemisphere_fibonacci_sampling
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint):
    ...
    if opt.normal_supervision:
        if opt.lambda_normal_gt <= 0:
            raise ValueError(
                "Stage1 --normal_supervision requires --lambda_normal_gt > 0")
        if not getattr(dataset, "pred_material_dir", ""):
            raise ValueError(
                "Stage1 --normal_supervision requires --pred_material_dir")
        print(f"[Stage1 normal GT] enabled: weight={opt.lambda_normal_gt}, "
              f"start>{opt.normal_loss_start}, source={dataset.pred_material_dir}")
    elif opt.lambda_normal_gt > 0:
        print("[Stage1 normal GT] disabled: --lambda_normal_gt is ignored "
              "unless --normal_supervision is set")
    ...
    while iteration < TOT_ITER:
        ...
        loss_opacity_reg = tb_dict["loss_opacity_reg"]
        ...
        with torch.no_grad():
            ...
            ema_opacity_reg_for_log = 0.4 * loss_opacity_reg + 0.6 * ema_opacity_reg_for_log
            ...
            if iteration % 10 == 0:
                loss_dict = {
                    ...
                    "OpacityReg": f"{ema_opacity_reg_for_log:.{5}f}",
            ...
            # tensorboard: scalars every iteration (same style as train.py),
            # so the geometry stage gets full loss curves + PSNR + GS count.
            if tb_writer:
                ...
                xyz_lr = next((g["lr"] for g in gaussians.optimizer.param_groups
                               if g["name"] == "xyz"), None)
                if xyz_lr is not None:
                    tb_writer.add_scalar("scene/xyz_lr", xyz_lr, iteration)
...
def save_training_vis(viewpoint_cam, gaussians, background, render_fn, pipe, opt, iteration, initial_stage):
    with torch.no_grad():
        ...
        # ---- 遮挡(visibility)图：与 Stage2 渲染方程完全相同的算法 ----
        # 复刻 gaussian_renderer/__init__.py render_ir 的 eval 分支：
        #   表面点   = surf_depth 反投影（rays_d_hw_unnormalized × depth + 相机中心）
        #   法线     = rend_normal / rend_alpha 归一化（2DGS 累积法线）
        #   入射方向 = 余弦加权低差异半球采样 diffuse_sample_num 条
        #              （random_rotate=False，与 Stage2 eval 一致）
        #   trace_origins = position + incident_dirs × light_t_min
        #   visibility = mean_S(1 - trace_alpha)，最后 × render_alpha 合成图
        # 注意：不能用 render_fn/render_initial 渲染 —— volume 阶段的
        # render_volume 根本不处理 override_color(函数体内无引用)，会返回
        # 正常光照渲染图(看起来跟 RGB 一样)。这里直接用与 Stage2 相同的
        # per-pixel trace，数值上就是 Stage2 那张 visibility 图。
        # 失败时降级为全 1（不阻断 visualize）。
        try:
            alpha_img = render_pkg["rend_alpha"]
            points = render_pkg["surf_depth"].permute(1, 2, 0) * \
                viewpoint_cam.rays_d_hw_unnormalized + viewpoint_cam.camera_center
            normal_map = render_pkg["rend_normal"].permute(1, 2, 0)
            normal_map = normal_map / alpha_img.permute(1, 2, 0).clamp_min(1e-6)
            normal_map = F.normalize(normal_map, dim=-1)
            mask = alpha_img[0] > 0
            pos = points[mask]
            nrm = normal_map[mask]
            # Stage1 的 PipelineParams 原本没有这两个属性（是 Stage2 的），
            # 用 getattr 兜底默认值与 Stage2 一致，避免老进程直接抛错。
            sample_num = getattr(pipe, 'diffuse_sample_num', 32)
            light_t_min = getattr(pipe, 'light_t_min', 0.10)
            incident_dirs, _ = cosine_hemisphere_fibonacci_sampling(
                nrm, sample_num, random_rotate=False)
            gaussians.build_bvh()
            # 与 Stage2 rendering_equation_chunk 相同的分块（每块 2**24/(diffuse+light)
            # 个像素），避免大图全量 trace 显存 OOM。
            chunk = 2**24 // sample_num
            vis = torch.empty(pos.shape[0], 1, device=pos.device)
            for i in range(0, pos.shape[0], chunk):
                dirs = incident_dirs[i:i + chunk]
                trace_origins = pos[i:i + chunk].unsqueeze(1) + dirs * light_t_min
                trace_alpha = gaussians.trace(
                    trace_origins, dirs,
                    camera_center=viewpoint_cam.camera_center)['alpha']
                vis[i:i + chunk] = (1 - trace_alpha[..., None]).mean(dim=1)
            vis_map = torch.zeros_like(render_pkg["render"][:1]).permute(1, 2, 0)
            vis_map[mask] = vis
            vis_map = vis_map.permute(2, 0, 1) * alpha_img
            vis_map = vis_map.repeat(3, 1, 1)
        except Exception as e:
            print(f"[visualize] 遮挡(visibility)图计算失败，降级为全白: {e}")
            vis_map = torch.ones_like(render_pkg["render"])
        visualization_list.append(vis_map)
