#!/bin/bash
# 重训 4 个受 rpy bug 影响的 link (数据已用 sxyz 重建, IoU=1.0 验证):
#   base_link shoulder_link upper_arm_link wrist_link
# × (4a Ref-GS 50000 → 4b IRGS 20000), 每 link 一卡并行, GPU 2/3/5/6
# 无伪监督: 不传 --pred_material_dir, lambda_albedo/roughness/metallic/normal_gt 全 0
set -u
source ~/miniconda3/etc/profile.d/conda.sh
conda activate irgs
export OPEN3D_CPU_RENDERING=true OPENCV_IO_ENABLE_OPENEXR=1

LINKS=(base_link shoulder_link upper_arm_link wrist_link)
GPUS=(2 3 5 6)
...
wait
echo "ALL 4 LINKS TRAINED (parallel) $(date +%H:%M:%S)"
