...
def cosine_hemisphere_fibonacci_sampling(normals, sample_num,
                                         random_rotate=True):
    """Low-discrepancy cosine-weighted hemisphere samples around normals."""
    pre_shape = normals.shape[:-1]
    normals = normals.reshape(-1, 3)
    point_count = normals.shape[0]

    idx = torch.arange(
        sample_num, dtype=normals.dtype, device=normals.device
    )[None] + 0.5
    radius = torch.sqrt(idx / sample_num)
    phi = np.pi * (3.0 - np.sqrt(5.0)) * idx
    if random_rotate:
        phi = phi + torch.rand(
            point_count, 1, dtype=normals.dtype, device=normals.device
        ) * (2 * np.pi)
    x = radius * torch.cos(phi)
    y = radius * torch.sin(phi)
    z = torch.sqrt((1.0 - radius.square()).clamp_min(0.0))
    local_samples = torch.stack((x, y, z.expand_as(x)), dim=-2)

    rotation_matrix = rotation_between_z(normals)
    incident_dirs = rotation_matrix @ local_samples
    incident_dirs = F.normalize(incident_dirs, dim=-2).transpose(-1, -2)
    pdf = z[..., None].clamp_min(1e-6) / np.pi
    incident_areas = (1.0 / pdf).expand(point_count, -1, -1)
    incident_dirs = incident_dirs.reshape(*pre_shape, sample_num, 3)
    incident_areas = incident_areas.reshape(*pre_shape, sample_num, 1)
    return incident_dirs, incident_areas
