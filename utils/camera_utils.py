...
from utils.graphics_utils import fov2focal, focal2fov, srgb_to_rgb
WARNED = False
...
def loadCam(args, id, cam_info, resolution_scale):
    ...
    # GT material maps (arb_render_data), normalized to RAW LINEAR at load:
    # the albedo PNG is sRGB-encoded -> convert with srgb_to_rgb; the orm PNG
    # is rendered Raw -> already linear [occlusion, roughness, metallic]
    gt_albedo = None
    if getattr(cam_info, "albedo", None) is not None:
        gt_albedo = srgb_to_rgb(PILtoTorch(cam_info.albedo, resolution).clamp(0.0, 1.0))
    gt_orm = PILtoTorch(cam_info.orm, resolution) if getattr(cam_info, "orm", None) is not None else None
    # GT normal is kept as float32 numpy (H, W, 3) in [-1, 1] world-space;
    # resize in float domain to avoid uint8 quantization artifacts.
    gt_normal = None
    if getattr(cam_info, "normal", None) is not None:
        n = cam_info.normal.astype(np.float32)
        if n.shape[:2] != resolution[::-1]:
            n = cv2.resize(n, resolution, interpolation=cv2.INTER_LINEAR)
        n = torch.from_numpy(n).permute(2, 0, 1)
        gt_normal = n / n.norm(dim=0, keepdim=True).clamp_min(1e-6)
    ...
    return Camera(colmap_id=cam_info.uid, R=cam_info.R, T=cam_info.T, 
                  ...
                  gt_albedo=gt_albedo, gt_orm=gt_orm, gt_normal=gt_normal)
